...
class LayerNorm3d(nn.Module):
    ...
    def forward(self, x):
        if not self.channels_first:
            ...
            #x = self.weight.reshape((1, -1, 1, 1, 1)) * x + self.bias.reshape((1, -1, 1, 1, 1))
...
class InceptManBlock3d(nn.Module):
    def __init__(self, dim, drop_path=0.0, square_kernel_size=7, band_kernel_size=11, branch_ratio=0.25):
        super().__init__()

        gc = int(dim * branch_ratio) # channel numbers of a convolution branch
        self.branch_ratio = branch_ratio
        self.dwconv_hwl = nn.Conv3d(gc, gc, kernel_size=square_kernel_size, padding=square_kernel_size//2, groups=gc)
        self.dwconv_wl = nn.Conv3d(gc, gc, kernel_size=(1, band_kernel_size, band_kernel_size),
                                   padding=(0, band_kernel_size//2, band_kernel_size//2), groups=gc)
        self.dwconv_hl = nn.Conv3d(gc, gc, kernel_size=(band_kernel_size, 1, band_kernel_size),
        ...
        if self.branch_ratio == 0.25:
          self.split_indexes = (gc, gc, gc, gc)      
        else:
          self.split_indexes = (gc, gc, gc, gc, dim - 4 * gc)                    
    ...
    def forward(self, x):
        input = x
        if self.branch_ratio != 0.25:
            x_hwl, x_wl, x_hl, x_hw, x_id = torch.split(x, self.split_indexes, dim=1)
            x = torch.cat((self.dwconv_hwl(x_hwl),self.dwconv_wl(x_wl),
                                                self.dwconv_hl(x_hl),self.dwconv_hw(x_hw),x_id), dim=1)
            
        else:
            x_hwl, x_wl, x_hl, x_hw = torch.split(x, self.split_indexes, dim=1)
            x = torch.cat((self.dwconv_hwl(x_hwl), self.dwconv_wl(x_wl), self.dwconv_hl(x_hl), self.dwconv_hw(x_hw)), dim=1)
...
class UNextV23d_inceptionNeXt(nn.Module):
    def __init__(self, in_channels=1, num_classes=2,
                 depths=[2, 2, 2, 2, 2], dims= [32, 64, 128, 64, 32],
                 drop_path_rate=0., head_init_scale=1., square_kernel_size=3, band_kernel_size=11,
                 branch_ratio=0.25
                 ):
        super().__init__()
        ...
        self.init_stage = nn.Sequential(
            ...
            #nn.Conv3d(in_channels, dims[0], kernel_size=1),
            InceptManBlock3d(dim=dims[0], drop_path=0.0, square_kernel_size=square_kernel_size,
                                  band_kernel_size=band_kernel_size, branch_ratio=branch_ratio)
        )
        ...
        for i in range(len(dims) // 2 + 1):
            stage = nn.Sequential(
                *[InceptManBlock3d(dim=dims[i], drop_path=dp_rates[cur + j], square_kernel_size=square_kernel_size,
                                        band_kernel_size=band_kernel_size, branch_ratio=branch_ratio) for j in range(depths[i])]
            )
        ...
        for i in range(len(dims) // 2):
            ...
            stage = nn.Sequential(
                *[InceptManBlock3d(dim=dims[i + len(dims) // 2 + 1], drop_path=dp_rates[cur + j], square_kernel_size=square_kernel_size,
                                        band_kernel_size=band_kernel_size, branch_ratio=branch_ratio) for j in range(num_blocks)]
            )
...
class InceptMan_m(UNextV23d_inceptionNeXt):
    def __init__(self, num_classes=2, square_kernel_size=3, band_kernel_size=11, gra=False, Shuffle=False, branch_ratio=0.25):
        super().__init__(in_channels=1,num_classes=num_classes,depths=[1,1,1,3,1,1,1,1,1],dims=[32, 64, 128, 256, 512, 256, 128, 64, 32],drop_path_rate=0.1,
                         square_kernel_size=square_kernel_size, band_kernel_size=band_kernel_size,
